sap.ui.define(["transener/GestionHabilitaciones/utils/FormatHelper",
				"transener/GestionHabilitaciones/utils/i18nTranslationHelper"], function (FormatHelper, Texts) {
	"use strict";
	return {
		_getmFunction: function (oModel, oModelFunciones) {
			const funciones = oModelFunciones.find(element => element.Codigo === oModel.Tipohab);
			if (funciones !== undefined){
				return funciones.Funcion;
			} else {
				return "-";
			}
		},
		_getRmodel: function (oModel, oModelRegiones) {
			const regiones = oModelRegiones.find(element => element.Codigo === oModel.Area);
			if (regiones !== undefined){
				return regiones.Region;
			} else {
				return "-";
			}
		},
		handlePrintAndDownload: function (oModel, oTipoHabilitacion, oModelHabilitacion, customData) {
			let dataPerfil = oModelHabilitacion.ImgPerfil,
				Habilitado_Nombre = oModelHabilitacion.Habilitado_Nombre ? oModelHabilitacion.Habilitado_Nombre : "-", 
				oPerfil = FormatHelper.formatImage(dataPerfil),
				oSignatureHabilitado = FormatHelper.formatImage(oModelHabilitacion.Habilitado_Firma),
				oSignatureMedico = FormatHelper.formatImage(oModelHabilitacion.Medico_Firma),
				oSignatureSyG = FormatHelper.formatImage(oModelHabilitacion.Seg_higiene_Firma),
				oSignatureJefeTct = FormatHelper.formatImage(oModelHabilitacion.Capacitacion_Firma),
				JefeTct_Nombre = oModelHabilitacion.Capacitacion_Nombre ? oModelHabilitacion.Capacitacion_Nombre : "-",
				Gerente_Nombre = oModelHabilitacion.Gerente_Nombre ? oModelHabilitacion.Gerente_Nombre : "-",
				personalInt = oModel.Interno ? 'Si' : 'No',
				nombApe = oModel.Nombre + " " + oModel.Apellido,
				fechaNacimiento = oModel.fechaNacimiento ? newArrayValues[i][11] : "-",
				grdAptitud = oModelHabilitacion.Grado_aptitud ? oModelHabilitacion.Grado_aptitud : "-",
				medicoFecha = oModelHabilitacion.Medico_Fecha ? FormatHelper.formateDate(oModelHabilitacion.Medico_Fecha) : "-",
				medicoObs = oModelHabilitacion.Medico_Observacion ? oModelHabilitacion.Medico_Observacion : "-",
				sygFecha = FormatHelper.formateDate(oModelHabilitacion.Seg_higiene_Fecha),
				sygObs = oModelHabilitacion.Seg_higiene_Observacion ? oModelHabilitacion.Seg_higiene_Observacion : "-",
				mto33 = oModel.Mto33 ? "X" : "",
				mto66 = oModel.Mto66 ? "X" : "",
				mto132 = oModel.Mto132 ? "X" : "",
				mto220 = oModel.Mto220 ? "X" : "",
				mto330 = oModel.Mto13 ? "X" : "",
				mto500 = oModel.Mto500 ? "X" : "",
				_base = customData.BaseDescription;
				
			let	_puesto = sap.ui.getCore().getModel("PuestosModel").getData().PuestosModel.find(element => element.Tipo == oModel.Puesto);
			let _puestoText = "-";
			if (_puesto !== undefined) { _puestoText = _puesto.Descripcion; } 
			
			let _gradoAptitud = customData.GradoApitud.find( element => element.Codigo == grdAptitud );	
			let _gradoAptitudText = "-";
			if (_gradoAptitud !== undefined) { _gradoAptitudText = _gradoAptitud.Grado; } 
			
			if (oModel.Hab_CETCT_nav[0]) { //Hago esto porque explotan algunas licencias y quedan cargando for ever cuando no encuentra sModel.Hab_CETCT_nav[0]
				oModel.FechaCapacitacion = FormatHelper.formateDate(oModel.Hab_CETCT_nav[0].FechaCapacitacion);
				oModel.TipoHab = oModel.Hab_CETCT_nav[0].Tipohab;
				oModel.Tension = oModel.Hab_CETCT_nav[0].Tension;
				oModel.Clase = oModel.Hab_CETCT_nav[0].Clase;
				oModel.Vencimiento = FormatHelper.formateDate(oModel.Hab_CETCT_nav[0].Vencimiento);
			} else {
				oModel.FechaCapacitacion = "-";
				oModel.TipoHab = "-";
				oModel.Tension = "-";
				oModel.Clase = "-";
				oModel.Vencimiento = "-";
			}
			let gerenteFecha = FormatHelper.formateDate(oModelHabilitacion.Gerente_Fecha),
				gerenteFirma = FormatHelper.formatImage(oModelHabilitacion.Gerente_Firma);
			let _content = [];
			let _contenido = [];
			
			//--------LOGOS--------
			let EmpresaLogo = "";
			if (oModel.Empresa === "TRANSENER") {
				EmpresaLogo =
					"data:image/png;base64,";
			} else {
				//TRANSBA
				EmpresaLogo =
					"data:image/png;base64,";
			}
			
			//----------- INICIO CONTENIDO DEL FORMULARIO ------------------//
			_contenido.push({
				margin: [0, 0, 0, 15],
				alignment: "center",
				table: {
					widths: ['*', '*'],
					body: [
						[{
							width: 100,
							image: EmpresaLogo,
							alignment: "center"
						}, {
							text: "Licencia Habilitante - Personal para TcT",
							style: "header",
							alignment: "center",
							margin: [0, 6, 0, 0]
						}]
					]
				}
			});
			
			_contenido.push({
				text: "\n" + "A) Datos personales del operario:",
				style: 'title'
			});
			
			_contenido.push({
				text: "\n" + "Personal Interno: " + personalInt + "\n" + 
							 "Legajo: " + oModel.Legajo + "\n" + 
						     "Nombre/s y Apellido/s: " + nombApe + "\n" +
						     "Documento: " + oModel.Documento + "\n" + 
						     "Fecha de Nacimiento: " + fechaNacimiento + "\n" + 
							 "Puesto: " + _puestoText + "\n" +
						     "Base: " + _base,
				style: 'anotherStyle'
			});
			_contenido.push({
				text: "\n" + "Conformidad personal para efectuar trabajos con tensión",
				style: 'title'
			}); 
			
			_contenido.push({
				text: Texts.getTranslation("descripOperario"),
				style: 'smallText'
			})
			
			//Firma del operario
			let _firmaOperario = [];
			_firmaOperario.push({ text: "\n" + "Firma del Operario",
							 	  style: 'signature' });
			if (oSignatureHabilitado !== "-") { 
				_firmaOperario.push({ image: oSignatureHabilitado,
							    	  fit: [150, 150],
							    	  style: 'signature' });
			}
			_contenido.push({
				columns: 
					[
						{
							text: "\n" + "Aclaración" + "\n" + Habilitado_Nombre,
							style: 'anotherStyle'
						}, 
						_firmaOperario
					]
			}); 
			
			//-------------------------B) Médico--------------------------------// 			
			_contenido.push({
				text: "\n" + "B) Aptitud Psicofísica - (Modificaciones en Archivo en papel o digital en Base de Datos)",
				style: 'title'
			}); 
			
			_contenido.push({
				text: Texts.getTranslation("SubTitleAptoMedico"),
				style: 'smallText'
			});
                                	              
			/*let _firmaMedico = [];
			_firmaMedico.push({ text: "\n" + "Salud Ocupacional ",
								style: 'signature' });
			if (oSignatureMedico !== "-") {								
				_firmaMedico.push({ image: oSignatureMedico,
									fit: [150, 150],
									style: 'signature' });
			}			*/			
			_contenido.push({ 
				columns:
				[
					{
					 	width: 'auto',
						text: "\n" + "Grado de aptitud: " + _gradoAptitudText + "\n" + 
									 "Fecha de Vencimiento: " + medicoFecha + "\n" + 
									 "Observaciones: " + medicoObs,
						style: 'anotherStyle'
					}, 
					//_firmaMedico
				]
			}); 
			
			//---------------------------C) Seguridad e Higiene-----------------------------------// 
			_contenido.push({
				text: "\n" + "C) Seguridad, Higiene en el Trabajo y Medio Ambiente",
				style: 'title'
			}); 
			_contenido.push({
				text: Texts.getTranslation("descripSeguridadHigiene"),
				style: 'smallText'
			});			
			
			// Firma del SHTMA
			/*let _firmaSHTMA = [];
			_firmaSHTMA.push({ text: "\n" + "SHTMA",	
							   style: 'signature' });
			if (oSignatureSyG !== "-"){							   
				_firmaSHTMA.push({ image: oSignatureSyG,
								   fit: [150, 150],
								   style: 'signature' });				
			}*/
			_contenido.push({
				columns: 
				[ 
					{ 
						text: "\n" + "Fecha de Vencimiento: " + sygFecha + "\n" + 
						             "Observaciones: " + sygObs,
						style: 'anotherStyle'
					}
					//_firmaSHTMA
				]
			});
			
			//-----------------------D) Trabajo con tensión----------------------------------------// 
			_contenido.push({
				text: "\n" + "D) Centro de Trabajos con Tensión - Capacitación y Entrenamiento",
				style: 'title'
			});
			_contenido.push({
				text: Texts.getTranslation("descripTrabajosTension"),
				style: 'smallText'
			});				
			
			//Firma y nombre del Jefe del Centro de TcT
			let _firmaTct = [];
			_firmaTct.push({ text: "\n" + "Jefe del Centro de TcT",	
							   style: 'signature' });
			//Issue 278 - saco la firma y dejo solo nombre
			/*if (oSignatureJefeTct !== "-"){							   
				_firmaTct.push({ image: oSignatureJefeTct,
								   fit: [150, 150],
								   style: 'signature' });				
			};*/
			_firmaTct.push({ text: "\n" + JefeTct_Nombre,	
							   style: 'signature' });
			
			_contenido.push(
				{ columns: 
					[
						{
							text: "\n" + "Clase: " + customData.Lote + "\n" +
									     "Tipo de Habilitación: " + customData.ClaseHabilitacion,
							style: 'anotherStyle'
						},
						_firmaTct
					]
				});
			
			_contenido.push(
				{
					columns: 
					[
						{	
							width: 100,
							text: "\n" + "Tensión",
							style: 'title'
						},
						{ 
							table: { body: [ ['33', '66', '132', '220', '330', '500'],
   									   	   [mto33, mto66, mto132, mto220, mto330, mto500] ] }
						}
					]
				}
			);
			
			//-------------------------E) Habilitación------------------------------------------// 
			_contenido.push({
				text: "\n" + "E) Habilitación",
				style: 'title'
			});
			_contenido.push({
				text: Texts.getTranslation("descriphabilitacionPersonal"),
				style: 'smallText'
			});	
			
			// Firma y nombre del gerente
			let _firmaGerente = [];
			_firmaGerente.push({ width: 100,
								 text: "\n" + "Gerente Regional",
								 style: 'signature' });
			//Issue 278 - saco la firma y dejo solo nombre
			/*if (gerenteFirma !== "-") {
				_firmaGerente.push({ image: gerenteFirma,
									 fit: [150, 150],
									 style: 'signature' });
			};*/
			_firmaGerente.push({ text: "\n" + Gerente_Nombre,	
							   style: 'signature' });
			
			_contenido.push(
				{
					columns: 
					[
						{
							text: "\n" + "Fecha: " + gerenteFecha,
							style: 'anotherStyle'
						},
						_firmaGerente
					]
				}
			); 
			
			//----------- FIN CONTENIDO DEL FORMULARIO ------------------//
			
			//Agregar el contenido al array final
			for (let i = 0; i < _contenido.length; i++) {
				if (_contenido[i].image !== "-") {
					_content.push(_contenido[i]);
				}
			};
			
			//Definición del formulario
			let docDefinition = {
				info: {
					title: 'Impresión Formulario TCT',
					author: 'Transener'
				},
				header: function () {
					if (oPerfil !== "-") {
						return {
							image: oPerfil,
							width: 50,
							height: 50,
							alignment: 'right',
							margin: [70, 80, 0, 0]
						};
					}
				},
				content: _content,
				styles: {
					header: {
						fontSize: 10,
						bold: true,
						alignment: 'justify',
						top: 4
					},
					smallText: {
						fontSize: 8,
						top: 2
					},
					title: {
						fontSize: 9,
						bold: true,
						background: '#E7F3FD'
					},
					anotherStyle: {
						color: '#515151',
						fontSize: 9,
						bold: true,
						top: 1,
						bottom: 1
					},
					defaultStyle: {
						top: 8
					},
					signature: {
						color: '#515151',
						fontSize: 9,
						bold: false,
						top: 0,
						alignment: 'center'
					}
				}
			};
			pdfMake.createPdf(docDefinition).open();
		}
		
	};
});