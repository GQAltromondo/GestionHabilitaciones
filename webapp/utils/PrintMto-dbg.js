sap.ui.define(["transener/GestionHabilitaciones/utils/FormatHelper",
	"transener/GestionHabilitaciones/utils/i18nTranslationHelper"
], function (FormatHelper, Texts) {
	"use strict";
	return {
		_getRegionArea: function (oModel, oModelRegiones) {
			const regiones = oModelRegiones.find(element => element.Codigo === oModel.Area);
			if (regiones !== undefined) {
				return regiones.Region;
			} else {
				return "-";
			}
		},
		_formatPuesto: function (tareasX) {
			if (tareasX.includes('EstSolic') || tareasX.includes('ProtSolic') || tareasX.includes('MedSolic') || tareasX.includes('SisContSolic') ||
				tareasX.includes('SisComuSolic') || tareasX.includes('LineaSolic') || tareasX.includes('AdmRedOpSolic')) {
				let Solicitante = "Solicitante";
				return Solicitante;
			} else if (tareasX.includes('EstJefeTraba') || tareasX.includes('ProtJefeTraba') || tareasX.includes('MedJefeTraba') || tareasX.includes(
					'SisContJefeTraba') ||
				tareasX.includes('SisComuJefeTraba') || tareasX.includes('LineaJefeTraba') || tareasX.includes('AdmRedOpJefeTraba')) {
				let JefeTrabajo = "Jefe de Trabajo";
				return JefeTrabajo;
			} else if (tareasX.includes('EstJefeTrabaSupl') || tareasX.includes('ProtJefeTrabaSupl') || tareasX.includes('MedJefeTrabaSupl') ||
				tareasX.includes('SisContJefeTrabaSupl') || tareasX.includes('SisComuJefeTrabaSupl') || tareasX.includes('LineaJefeTrabaSupl') ||
				tareasX.includes('AdmRedOpJefeTrabaSupl')) {
				let JefeTrabajoGuiado = "Jefe de Trabajo Guiado";
				return JefeTrabajoGuiado;
			} else if (tareasX.includes('EstPersonalMant') || tareasX.includes('ProtPersonalMant') || tareasX.includes('MedPersonalMant') ||
				tareasX.includes('SisContPersonalMant') || tareasX.includes('SisComuPersonalMant') || tareasX.includes('LineaPersonalMant') ||
				tareasX.includes('AdmRedOpPersonalMant')) {
				let PersonalMto = "Personal de Mto.";
				return PersonalMto;
			}
			return "";
		},
		_formatArea: function (tareasX) {
			if (tareasX.includes('EstSolic') || tareasX.includes('EstJefeTraba') || tareasX.includes('EstJefeTrabaSupl') || tareasX.includes(
					'EstPersonalMant')) {
				let EstTrans = "Estaciones Transformadoras";
				return EstTrans;
			} else if (tareasX.includes('ProtSolic') || tareasX.includes('ProtJefeTraba') || tareasX.includes('ProtJefeTrabaSupl') || tareasX.includes(
					'ProtPersonalMant')) {
				let Protecciones = "Protecciones";
				return Protecciones;
			} else if (tareasX.includes('MedSolic') || tareasX.includes('MedJefeTraba') || tareasX.includes('MedJefeTrabaSupl') || tareasX.includes(
					'MedPersonalMant')) {
				let Mediciones = "Mediciones";
				return Mediciones;
			} else if (tareasX.includes('SisContSolic') || tareasX.includes('SisContJefeTraba') || tareasX.includes('SisContJefeTrabaSupl') ||
				tareasX.includes('SisContPersonalMant')) {
				let SistControl = "Sistemas de Control";
				return SistControl;
			} else if (tareasX.includes('SisComuSolic') || tareasX.includes('SisComuJefeTraba') || tareasX.includes('SisComuJefeTrabaSupl') ||
				tareasX.includes('SisComuPersonalMant')) {
				let SistCom = "Sistemas de Comunicaciones";
				return SistCom;
			} else if (tareasX.includes('LineaSolic') || tareasX.includes('LineaJefeTraba') || tareasX.includes('LineaJefeTrabaSupl') || tareasX.includes(
					'LineaPersonalMant')) {
				let Lineas = "Líneas";
				return Lineas;
			} else if (tareasX.includes('AdmRedOpSolic') || tareasX.includes('AdmRedOpJefeTraba') || tareasX.includes('AdmRedOpJefeTrabaSupl') ||
				tareasX.includes('AdmRedOpPersonalMant')) {
				let AdmRedes = "Adm. de Redes de Operación";
				return AdmRedes;
			}
			return "";
		},
		_formatTension: function (tensionX) {
			let tensiones = [];
			if (tensionX.includes("Mto500")) {
				let Mto500 = "500 kV - ";
				tensiones.push(Mto500);
				//return Mto500;
			} if (tensionX.includes("Mto220")) {
				let Mto220 = "220 kV - ";
				tensiones.push(Mto220);
				//return Mto220;
			} if (tensionX.includes("Mto132")) {
				let Mto132 = "132 kV - ";
				tensiones.push(Mto132);
				//return Mto132;
			} if (tensionX.includes("Mto66")) {
				let Mto66 = "66 kV - ";
				tensiones.push(Mto66);
				//return Mto66;
			} if (tensionX.includes("Mto33")) {
				let Mto33 = "33 kV - ";
				tensiones.push(Mto33);
				//return Mto33;
			} if (tensionX.includes("Mto13")) {
				let Mto13 = "13 kV";
				tensiones.push(Mto13);
				//return Mto13;
			}
			return tensiones;
		},
		handlePrintAndDownload: function (oModel, oModelHabilitacion, oRegionModel, taskSelectionModel, SendCommentModel) {
			let oModelGral = oModel.getData();
			let oModelHab = oModelHabilitacion.getData();
			let oSendCommentModel = SendCommentModel.getData();
			let oModelRegiones = oRegionModel.getData().Regiones;
			let datosAgente = {
				personalInterno: oModelGral.Interno ? 'Si' : 'No',
				personalLegajo: oModelGral.Legajo ? oModelGral.Legajo : "-",
				personalNomApe: oModelGral.Apellido + ", " + oModelGral.Nombre,
				personalDNI: oModelGral.Documento ? oModelGral.Documento : "-",
				personaRegArea: this._getRegionArea(oModelGral, oModelRegiones)
			};
			//Seccion 1 formulario - Datos personales del agente
			let {
				personalInterno,
				personalLegajo,
				personalNomApe,
				personalDNI,
				personaRegArea
			} = datosAgente;
			//Seccion 2 formulario - Conformidad del agente
			let conformidadDelAgente = {
				Solicitante_Fecha: FormatHelper.formateDate(oModelHab.Solicitante_Fecha),
				Solicitante_Nombre: oModelHab.Solicitante_Nombre ? oModelHab.Solicitante_Nombre : "-",
				Solicitante_Firma: FormatHelper.formatImage(oModelHab.Solicitante_Firma),
				SendCommentModel: oSendCommentModel.ConformidadAgenteComment ? oSendCommentModel.ConformidadAgenteComment : "-"
			};
			//Seccion 3 formulario - Capacitación y Entrenamiento
			let entExpPreviaTrabajo = {
				Capacitacion_Fecha: FormatHelper.formateDate(oModelHab.Capacitacion_Fecha),
				Capacitacion_Nombre: oModelHab.Capacitacion_Nombre ? oModelHab.Capacitacion_Nombre : "-",
				Capacitacion_Firma: FormatHelper.formatImage(oModelHab.Capacitacion_Firma),
				CapaYEntrenamComment: oSendCommentModel.CapaYEntrenamComment ? oSendCommentModel.CapaYEntrenamComment : "-"
			};
			//Seccion 4 formulario - Habilitación (tabla de kv)
			let {
				Mto500,
				Mto220,
				Mto132,
				Mto66,
				Mto33,
				Mto13
			} = oModelGral;
			let tension = {
				Mto500: Mto500 ? "X" : "-",
				Mto220: Mto220 ? "X" : "-",
				Mto132: Mto132 ? "X" : "-",
				Mto66: Mto66 ? "X" : "-",
				Mto33: Mto33 ? "X" : "-",
				Mto13: Mto13 ? "X" : "-"
			};
			//Seccion 4 formulario - Tareas
			let tareas = {
				//Estaciones transformadoras
				EstSolic: taskSelectionModel.getProperty("/EstSolic/selected") ? "X" : " ",
				EstJefeTraba: taskSelectionModel.getProperty("/EstJefeTraba/selected") ? "X" : " ",
				EstJefeTrabaSupl: taskSelectionModel.getProperty("/EstJefeTrabaSupl/selected") ? "X" : " ",
				EstPersonalMant: taskSelectionModel.getProperty("/EstPersonalMant/selected") ? "X" : " ",
				//Protecciones
				ProtSolic: taskSelectionModel.getProperty("/ProtSolic/selected") ? "X" : " ",
				ProtJefeTraba: taskSelectionModel.getProperty("/ProtJefeTraba/selected") ? "X" : " ",
				ProtJefeTrabaSupl: taskSelectionModel.getProperty("/ProtJefeTrabaSupl/selected") ? "X" : " ",
				ProtPersonalMant: taskSelectionModel.getProperty("/ProtPersonalMant/selected") ? "X" : " ",
				//Mediciones
				MedSolic: taskSelectionModel.getProperty("/MedSolic/selected") ? "X" : " ",
				MedJefeTraba: taskSelectionModel.getProperty("/MedJefeTraba/selected") ? "X" : " ",
				MedJefeTrabaSupl: taskSelectionModel.getProperty("/MedJefeTrabaSupl/selected") ? "X" : " ",
				MedPersonalMant: taskSelectionModel.getProperty("/MedPersonalMant/selected") ? "X" : " ",
				//Sistemas de control
				SisContSolic: taskSelectionModel.getProperty("/SisContSolic/selected") ? "X" : " ",
				SisContJefeTraba: taskSelectionModel.getProperty("/SisContJefeTraba/selected") ? "X" : " ",
				SisContJefeTrabaSupl: taskSelectionModel.getProperty("/SisContJefeTrabaSupl/selected") ? "X" : " ",
				SisContPersonalMant: taskSelectionModel.getProperty("/SisContPersonalMant/selected") ? "X" : " ",
				//Sistemas de comunicaciones
				SisComuSolic: taskSelectionModel.getProperty("/SisComuSolic/selected") ? "X" : " ",
				SisComuJefeTraba: taskSelectionModel.getProperty("/SisComuJefeTraba/selected") ? "X" : " ",
				SisComuJefeTrabaSupl: taskSelectionModel.getProperty("/SisComuJefeTrabaSupl/selected") ? "X" : " ",
				SisComuPersonalMant: taskSelectionModel.getProperty("/SisComuPersonalMant/selected") ? "X" : " ",
				//Líneas
				LineaSolic: taskSelectionModel.getProperty("/LineaSolic/selected") ? "X" : " ",
				LineaJefeTraba: taskSelectionModel.getProperty("/LineaJefeTraba/selected") ? "X" : " ",
				LineaJefeTrabaSupl: taskSelectionModel.getProperty("/LineaJefeTrabaSupl/selected") ? "X" : " ",
				LineaPersonalMant: taskSelectionModel.getProperty("/LineaPersonalMant/selected") ? "X" : " ",
				//Administración de redes de operación
				AdmRedOpSolic: taskSelectionModel.getProperty("/AdmRedOpSolic/selected") ? "X" : " ",
				AdmRedOpJefeTraba: taskSelectionModel.getProperty("/AdmRedOpJefeTraba/selected") ? "X" : " ",
				AdmRedOpJefeTrabaSupl: taskSelectionModel.getProperty("/AdmRedOpJefeTrabaSupl/selected") ? "X" : " ",
				AdmRedOpPersonalMant: taskSelectionModel.getProperty("/AdmRedOpPersonalMant/selected") ? "X" : " "
			};
			//Guardo los nombres de las tareas que tienen una X para usarlos en la impresión de conformidad del agente y capacitación y entrenamiento
			let tareasX = [];
			Object.entries(tareas).forEach(function ([key, value]) {
				if (value === 'X') {
					tareasX.push(key)
				}
			});
			//Guardo las tensiones tienen una X para usarlos en la impresión de conformidad del gerente
			let tensionX = [];
			Object.entries(tension).forEach(function ([key, value]) {
				if (value === 'X') {
					tensionX.push(key)
				}
			});
			
			let bTensionX;
			Object.entries(tension).forEach(function ([key, value]) {
				if (value === 'X') {
					bTensionX = true;
				}
			});
			
			//Sección 5 formulario - Conformidad del SSTMA
			let confSSTMA = {
				SegHigiene_Fecha: FormatHelper.formateDate(oModelHab.SegHigiene_Fecha),
				SegHigiene_Nombre: oModelHab.SegHigiene_Nombre ? oModelHab.SegHigiene_Nombre : "-",
				SegHigiene_Firma: FormatHelper.formatImage(oModelHab.SegHigiene_Firma),
				SeguridadHigieneComment: oSendCommentModel.SeguridadHigieneComment ? oSendCommentModel.SeguridadHigieneComment : "-"
			};
			//Sección 6 formulario - Conformidad de seguridad pública
			let confSegPublica = {
				SegPublica_Fecha: FormatHelper.formateDate(oModelHab.SegPublica_Fecha),
				SegPublica_Nombre: oModelHab.SegPublica_Nombre ? oModelHab.SegPublica_Nombre : "-",
				SegPublica_Firma: FormatHelper.formatImage(oModelHab.SegPublica_Firma),
				SegPublicaComment: oSendCommentModel.SegPublicaComment ? oSendCommentModel.SegPublicaComment : "-"
			};
			//Sección 7 FINAL formulario - Conformidad del Gerente Regional o Gerente de Planifcación u operación de la red
			let conformidadGerentes = {
				GerRegional_Fecha: FormatHelper.formateDate(oModelHab.GerRegional_Fecha),
				GerRegional_Fecha_vencimiento: FormatHelper.formateDate(oModelHab.GerRegional_Fecha_vencimiento),
				GerRegional_Nombre: oModelHab.GerRegional_Nombre ? oModelHab.GerRegional_Nombre : "-",
				GerRegional_Firma: FormatHelper.formatImage(oModelHab.GerRegional_Firma),
				ConfGerReg: oSendCommentModel.ConfGerReg ? oSendCommentModel.ConfGerReg : "-"
			};
			let _content = [];
			let _contenido = [];
			//Logo
			let EmpresaLogo = "";
			if (oModelGral.Empresa === "TRANSENER") {
				EmpresaLogo =
					"data:image/png;base64,";
			} else {
				//TRANSBA
				EmpresaLogo =
					"data:image/png;base64,";
			}
			// ----------- INICIO CONTENIDO DEL FORMULARIO ------------------ //
			_contenido.push({
				margin: [0, 0, 0, 20],
				alignment: "center",
				table: {
					widths: ['*', '*', '*'],
					body: [
						[{
							text: "Registro del Procedimiento Específico 09 M SIG 01 02 00",
							style: "subHeader",
							alignment: "center",
							margin: [0, 6, 0, 0]
						}, {
							text: [{
								text: "LICENCIA HABILITANTE\n",
								style: "header",
								decoration: "underline",
								alignment: "center"
							}, {
								text: "Personal de Mantenimiento",
								style: "header",
								alignment: "center"
							}],
							margin: [0, 6, 0, 0]
						}, {
							width: 100,
							image: EmpresaLogo,
							alignment: "center"
						}]
					]
				}
			});
			//---------- Datos personales del agente -------------//
			_contenido.push({
				text: "Datos Personales del Agente",
				style: 'title'
			});
			_contenido.push({
				text: "Apellido/s y Nombre/s: " + personalNomApe + "\n" +
					"Doc. de Identidad: " + personalDNI + "\n" + "Región, ARO o Gerencia: " + personaRegArea + "\n",
				style: 'subHeader'
			});
			//----------- Conformidad del Agente ------------------//
			_contenido.push({
				text: "\n" + "Conformidad del Agente",
				style: 'title'
			});
			_contenido.push({
				width: 'auto',
				text: "De acuerdo con las Normas, Reglamentos, Instrucciones y Recomendaciones establecidas por TRANSENER S.A. y a efectos de la pertinente habilitación, presto mi conformidad para actuar como: " +
					this._formatPuesto(tareasX) + "." + "\n" +
					"Firmado digitalmente: " + conformidadDelAgente.Solicitante_Nombre,
				style: 'subHeader'
			});
			//------- Capacitación y entrenamiento ----------//
			_contenido.push({
				text: "\n" + "Capacitación y Entrenamiento",
				style: 'title'
			});
			_contenido.push({
				width: 'auto',
				text: "Certifico que el Agente cuyos datos personales figuran al inicio, ha sido capacitado y entrenado para la actividad de: " +
					this._formatPuesto(tareasX) + "." + "\n" +
					"Firmado digitalmente: " + entExpPreviaTrabajo.Capacitacion_Nombre,
				style: 'subHeader'
			});
			//6 sept 2022 - SE PIDIO SACAR ESTO//
			//--------- Habilitación ---------//
			/*_contenido.push({
				text: "\n" + "HABILITACIÓN OTORGADA: " + this._formatPuesto(tareasX) + " - " + this._formatArea(tareasX),
				fontSize: 10,
				bold: true,
				color: '#1060ba'
			});
			_contenido.push({
				text: "\n" + '(Sólo para Líneas) NIVEL DE TENSIÓN PARA EL CUAL SE LO HABILITA (Tildar con una X)',
				style: 'itemsColumn'
			});
			_contenido.push({
				table: {
					widths: ['auto', 'auto', 'auto', 'auto', 'auto', 'auto'],
					body: [
						['500 KV', '220 KV', '132 KV', '66 KV', '33 KV', '13,2 KV'],
						[{text: tension.Mto500, alignment: 'center'}, {text: tension.Mto220, alignment: 'center'}, {text: tension.Mto132, alignment: 'center'}, {text: tension.Mto66, alignment: 'center'},
						 {text: tension.Mto33, alignment: 'center'}, {text: tension.Mto13, alignment: 'center'}]
					]
				}
			});*/
			//----------- Conformidad de SSTMA  ----------------------//
			_contenido.push({
				text: "\n" + "Conformidad de SSTMA",
				style: 'title'
			});
			_contenido.push({
				text: "Firmado digitalmente: " + confSSTMA.SegHigiene_Nombre,
				style: 'subHeader'
			});
			//----------- Conformidad de Seguridad Pública  ------------------//
			_contenido.push({
				text: "\n" + "Conformidad de Seg. Pública",
				style: 'title'
			});
			_contenido.push({
				text: "Firmado digitalmente: " + confSegPublica.SegPublica_Nombre,
				style: 'subHeader'
			});
			//----------- Seccion FINAL conformidad gerente Regional o gerente de Planificacion y Operación de la red  --------------------//
			_contenido.push({
				text: "\n" + "Conformidad del Gerente Regional, Gerente de Coordinación Técnica o Gerente de Planificación y Operación de la Red",
				style: 'title'
			});
			_contenido.push({
				text: [{
						text: "Habiendo considerado todos los datos precedentes y demás información relacionada, se otorga la correspondiente HABILITACIÓN OTORGADA: ",
						style: 'subHeader'
					}, {
						text: this._formatPuesto(tareasX) + " - ",
						bold: true,
						fontSize: 11
					}, {
						text: this._formatArea(tareasX),
						bold: true,
						fontSize: 11
					}]
					//text: "Habiendo considerado todos los datos precedentes y demás información relacionada, se otorga la correspondiente HABILITACIÓN OTORGADA:.",
					//style: 'subHeader'
			}, {
				text: [{
					text: "NIVEL DE TENSIÓN PARA EL CUAL SE LO HABILITA: ",
					style: 'subHeader'
				}, {
					text: bTensionX ? this._formatTension(tensionX) : "No aplica.",
					//text: bTensionX ? this._formatTension(tensionX) : "No aplica.",
					bold: true,
					fontSize: 11
				}]
			}, {
				text: [{
					text: "Habilitación válida hasta la fecha: ",
					style: 'subHeader'
				}, {
					text: conformidadGerentes.GerRegional_Fecha_vencimiento,
					bold: true,
					fontSize: 11
				}]
			}, {
				text: [{
					text: "Firmado digitalmente: ",
					style: 'subHeader'
				}, {
					text: conformidadGerentes.GerRegional_Nombre,
					bold: true,
					fontSize: 11
				}]
				//text: "Firmado digitalmente: " + conformidadGerentes.GerRegional_Nombre,
				//style: 'subHeader'
			});
			//------------ Agregar el contenido al array final -----------------//
			for (let i = 0; i < _contenido.length; i++) {
				if (_contenido[i].image != "-") {
					_content.push(_contenido[i]);
				}
			};
			//-------- Estructura del Formulario --------//
			let docDefinition = {
				info: {
					title: 'LICENCIA HABILITANTE (Personal de Mantenimiento)',
					author: 'Transener'
				},
				content: _content,
				styles: {
					header: {
						fontSize: 10,
						bold: true,
						alignment: 'justify',
						top: 4
					},
					smallText: {
						fontSize: 9,
						top: 2
					},
					title: {
						fontSize: 10,
						bold: true,
						color: '#1060ba',
						decoration: 'underline'
					},
					subHeader: {
						fontSize: 10,
						bold: false
					},
					anotherStyle: {
						color: '#515151',
						fontSize: 10,
						bold: true
					},
					defaultStyle: {
						top: 10
					},
					itemsColumn: {
						fontSize: 9,
						bold: true
					},
					signature: {
						color: '#000',
						fontSize: 10,
						bold: false,
						top: 0,
						alignment: 'center'
					},
					spacer: {
						top: 10,
						margin: 10
					},
					signatureEntrenamiento: {
						margin: [20, 0, 0, 0]
					}
				}
			};
			pdfMake.createPdf(docDefinition).open();
		}
	};
});