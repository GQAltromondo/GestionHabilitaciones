sap.ui.define(["transener/GestionHabilitaciones/utils/FormatHelper",
	"transener/GestionHabilitaciones/utils/i18nTranslationHelper"
], function (FormatHelper, Texts) {
	"use strict";
	return {
		_getmFunction: function (oModel, oModelFunciones) {
			const funciones = oModelFunciones.find(element => element.Codigo === oModel.Tipohab);
			if (funciones !== undefined){
				return funciones.Funcion;
			} else {
				return "-";
			}
		},
		_getRegionArea: function (oModel, oModelRegiones) {
			const regiones = oModelRegiones.find(element => element.Codigo === oModel.Area);
			if (regiones !== undefined){
				return regiones.Region;
			} else {
				return "-";
			}
		},
		
		_getGradoApto: function (oModel, SendCommentModel) {
			const gradosapt = SendCommentModel.find(element => element.Codigo === oModel.Hab_apmedico_nav[0].Gradoap);
			if (gradosapt !== undefined){
				return gradosapt.Grado;
			} else {
				return "-";
			}
		},
		handlePrintAndDownload: function (oModel, oTipoHabilitacion, oModelHabilitacion, oRegionModel, SendCommentModel) {
			let oModelGral = oModel;
			let oModelFunciones = oTipoHabilitacion;
			let oModelHab = oModelHabilitacion;
			//Seccion 2 formulario
			let datosAgente = {
				personalInterno: oModelGral.Interno ? 'Si' : 'No',
				personalLegajo: oModelGral.Legajo ? oModelGral.Legajo : "-",
				personalNomApe: oModelGral.Nombre + " " + oModelGral.Apellido,
				personalFechaNac: oModelGral.fechaNac ? oModel.fechaNac : "-",
				personalDNI: oModelGral.Documento ? oModelGral.Documento : "-",
				personaRegArea: this._getRegionArea(oModelGral, oRegionModel),
				personaFuncion: this._getmFunction(oModelGral, oModelFunciones)
			};
			//Seccion 3 formulario
			let entExpPreviaTrabajo = {
				Solicitador_Fecha: FormatHelper.formateDate(oModelHab.Solicitador_Fecha),
				Solicitador_Nombre: oModelHab.Solicitador_Nombre ? oModelHab.Solicitador_Nombre : "-",
				Solicitador_Firma: FormatHelper.formatImage(oModelHab.Solicitador_Firma)
			};
			//Seccion 4 formulario
			let conformidadEmpleado = {
				Habilitado_Fecha: FormatHelper.formateDate(oModelHab.Habilitado_Fecha),
				Habilitado_Nombre: oModelHab.Habilitado_Nombre ? oModelHab.Habilitado_Nombre : "-",
				Habilitado_Firma: FormatHelper.formatImage(oModelHab.Habilitado_Firma)
			};
			//Seccion 5 formulario
			let evalDesempempeño = {
				Capacitador_Fecha: FormatHelper.formateDate(oModelHab.Capacitador_Fecha),
				Capacitador_Nombre: oModelHab.Capacitador_Nombre ? oModelHab.Capacitador_Nombre : "-",
				Capacitador_Firma: FormatHelper.formatImage(oModelHab.Capacitador_Firma)
			};
			//Seccion 6 formulario
			let gradoAptitud = {
				//GradoAp: oModelHab.Gradoap ? oModelHab.Gradoap : "-",
				GradoAp: this._getGradoApto(oModelGral, SendCommentModel),
				Medicina_Fecha_vencimiento: FormatHelper.formateDate(oModelHab.Medicina_Fecha_vencimiento),
				Medicina_Fecha: FormatHelper.formateDate(oModelHab.Medicina_Fecha),
				Medicina_Nombre: oModelHab.Medicina_Nombre ? oModelHab.Medicina_Nombre : "-",
				Medicina_Firma: FormatHelper.formatImage(oModelHab.Medicina_Firma)
			};
			//Seccion 7 formulario
			let segHigiene = {
				aprobadoCheck: oModelHab.aprobadoCheck ? "Si" : "No",
				CountEvaluation: oModelHab.CountEvaluation ? oModelHab.CountEvaluation : "-",
				SeguridadHigiene_Fecha_examen: FormatHelper.formateDate(oModelHab.SeguridadHigiene_Fecha_examen),
				SeguridadHigiene_Fecha: FormatHelper.formateDate(oModelHab.SeguridadHigiene_Fecha),
				SeguridadHigiene_Anio_Venc: oModelHab.SeguridadHigiene_Anio_Venc ? oModelHab.SeguridadHigiene_Anio_Venc : "-",
				SeguridadHigiene_Cantidad_Venc: oModelHab.SeguridadHigiene_Cantidad_Venc ? oModelHab.SeguridadHigiene_Cantidad_Venc : "-",
				SeguridadHigiene_Tiempo_Venc: oModelHab.SeguridadHigiene_Tiempo_Venc ? oModelHab.SeguridadHigiene_Tiempo_Venc : "-",
				SeguridadHigiene_Observacion: oModelHab.SeguridadHigiene_Observacion ? oModelHab.SeguridadHigiene_Observacion : "-",
				SeguridadHigiene_Nombre: oModelHab.SeguridadHigiene_Nombre ? oModelHab.SeguridadHigiene_Nombre : "-",
				SeguridadHigiene_Firma: FormatHelper.formatImage(oModelHab.SeguridadHigiene_Firma)
			};
			//Seccion 8 formulario
			let evalClasificacion = {
				Examinador1_Nombre: oModelHab.Examinador1_Nombre ? oModelHab.Examinador1_Nombre : "-",
				Examinador1_Fecha: FormatHelper.formateDate(oModelHab.Examinador1_Fecha),
				Examinador1_Firma: FormatHelper.formatImage(oModelHab.Examinador1_Firma),
				Examinador2_Nombre: oModelHab.Examinador2_Nombre ? oModelHab.Examinador2_Nombre : "-",
				Examinador2_Fecha: FormatHelper.formateDate(oModelHab.Examinador2_Fecha),
				Examinador2_Firma: FormatHelper.formatImage(oModelHab.Examinador2_Firma),
				Examinador3_Nombre: oModelHab.Examinador3_Nombre ? oModelHab.Examinador3_Nombre : "-",
				Examinador3_Fecha: FormatHelper.formateDate(oModelHab.Examinador3_Fecha),
				Examinador3_Firma: FormatHelper.formatImage(oModelHab.Examinador3_Firma)
			};
			let gerRegionalJefCOT = {
				Gerente_Reg_Fecha: FormatHelper.formateDate(oModelHab.Gerente_Reg_Fecha),
				Gerente_Reg_Anio_Venc: oModelHab.Gerente_Reg_Anio_Venc ? oModelHab.Gerente_Reg_Anio_Venc : "-",
				Gerente_Reg_Cantidad_Venc: oModelHab.Gerente_Reg_Cantidad_Venc ? oModelHab.Gerente_Reg_Cantidad_Venc : "-",
				Gerente_Reg_Tiempo_Venc: oModelHab.Gerente_Reg_Tiempo_Venc ? oModelHab.Gerente_Reg_Tiempo_Venc : "-",
				Gerente_Reg_Observacion: oModelHab.Gerente_Reg_Observacion ? oModelHab.Gerente_Reg_Observacion : "-",
				Gerente_Reg_Nombre: oModelHab.Gerente_Reg_Nombre ? oModelHab.Gerente_Reg_Nombre : "-",
				Gerente_Reg_Firma: FormatHelper.formatImage(oModelHab.SeguridadHigiene_Firma)
			};
			let gerPlanOperRed = {
				Ger_Operaciones_Fecha: FormatHelper.formateDate(oModelHab.Ger_Operaciones_Fecha),
				Ger_Operaciones_Anio_Venc: oModelHab.Ger_Operaciones_Anio_Venc ? oModelHab.Ger_Operaciones_Anio_Venc : "-",
				Ger_Operaciones_Cantidad_Venc: oModelHab.Ger_Operaciones_Cantidad_Venc ? oModelHab.Ger_Operaciones_Cantidad_Venc : "-",
				Ger_Operaciones_Tiempo_Venc: oModelHab.Ger_Operaciones_Tiempo_Venc ? oModelHab.Ger_Operaciones_Tiempo_Venc : "-",
				Ger_Operaciones_Observacion: oModelHab.Ger_Operaciones_Observacion ? oModelHab.Ger_Operaciones_Observacion : "-",
				Ger_Operaciones_Nombre: oModelHab.Ger_Operaciones_Nombre ? oModelHab.Ger_Operaciones_Nombre : "-",
				Ger_Operaciones_Firma: FormatHelper.formatImage(oModelHab.Ger_Operaciones_Firma)
			};
			let gestCalidad = {
				ObservacionGestionCalidad: oModelHab.ObservacionGestionCalidad ? oModelHab.ObservacionGestionCalidad : "-",
				Gestion_Calidad_Fecha_Habilitacion: FormatHelper.formateDate(oModelHab.Gestion_Calidad_Fecha_Habilitacion),
				Gestion_Calidad_Nombre: oModelHab.Gestion_Calidad_Nombre ? oModelHab.Gestion_Calidad_Nombre : "-",
				Gestion_Calidad_Firma: FormatHelper.formatImage(oModelHab.Gestion_Calidad_Firma)
			};
			let verJefaturaGyC = {
				Rep_Direccion_Observacion: oModelHab.Rep_Direccion_Observacion ? oModelHab.Rep_Direccion_Observacion : "-",
				Rep_Direccion_Fecha: FormatHelper.formateDate(oModelHab.Rep_Direccion_Fecha),
				Rep_Direccion_Nombre: oModelHab.Rep_Direccion_Nombre ? oModelHab.Rep_Direccion_Nombre : "-",
				Rep_Direccion_Firma: FormatHelper.formatImage(oModelHab.Rep_Direccion_Firma)
			};
			let confDireTecnica = {
				Direccion_Tecnica_Fecha_Vigencia: FormatHelper.formateDate(oModelHab.Direccion_Tecnica_Fecha_Vigencia),
				Direccion_Tecnica_Fecha: FormatHelper.formateDate(oModelHab.Direccion_Tecnica_Fecha),
				Direccion_Tecnica_Nombre: oModelHab.Direccion_Tecnica_Nombre ? oModelHab.Direccion_Tecnica_Nombre : "-",
				Direccion_Tecnica_Firma: FormatHelper.formatImage(oModelHab.Direccion_Tecnica_Firma)
			};
			let _content = [];
			let _contenido = [];
			let EmpresaLogo = "";
			if (oModelGral.Empresa === "TRANSENER") {
				EmpresaLogo =
					"data:image/png;base64,";
			} else {
				//TRANSBA
				EmpresaLogo =
					"data:image/png;base64,";
			}
			
			// ----------- INICIO CONTENIDO DEL FORMULARIO ------------------ //
			_contenido.push({
				margin: [0, 0, 0, 20],
				alignment: "center",
				table: {
					widths: ['*', '*'],
					body: [
						[{
							width: 100,
							image: EmpresaLogo,
							alignment: "center"
						}, {
							text: "Licencia Habilitante - Personal de Operaciones",
							style: "header",
							alignment: "center",
							margin: [0, 6, 0, 0]
						}]
					]
				}
			});
			/*_contenido.push({
				text: 'Licencia Habilitante - Personal de Operaciones',
				style: 'header',
				alignment: 'center',
				margin: [0, -30, 0, 0]
			});*/
			_contenido.push({
				text: "\n" + "Función: " + datosAgente.personaFuncion,
				style: 'title'
			});
			_contenido.push({
				text: "\n" + "A) Datos Personales del Empleado:",
				style: 'title'
			});
			_contenido.push({
				text: "\n" + "Personal Interno: " + datosAgente.personalInterno + "\n" +
					"Legajo: " + datosAgente.personalLegajo + "\n" +
					"Nombre/s y Apellido/s: " + datosAgente.personalNomApe + "\n" +
					"Doc. de Identidad: " + datosAgente.personalDNI + "\n" +
					"Region / Area: " + datosAgente.personaRegArea + "\n",
				style: 'itemsColumn'
			});
			// -------   Entrenamiento / Experiencia previa en el trabajo ---------- //
			_contenido.push({
				text: "\n" + "Entrenamiento / Experiencia Previa en el Trabajo",
				style: 'title'
			});
			_contenido.push({
				text: "\n" +
					"Certifico que el Agente cuyos datos figuran al inicio, ha sido capacitado y entrado para la actividad",
				style: 'subTitle'
			});
			let _entExpPreviaTrabajo = [];
			if (entExpPreviaTrabajo.Solicitador_Firma !== "-") {
				_entExpPreviaTrabajo.push({
					image: entExpPreviaTrabajo.Solicitador_Firma,
					fit: [150, 150],
					style: 'signatureEntrenamiento'
				});
			}
			_contenido.push({
				columns: [{
						width: 'auto',
						text: "\n" +
							"Fecha: " + entExpPreviaTrabajo.Solicitador_Fecha + "\n" +
							"Aclaración: " + entExpPreviaTrabajo.Solicitador_Nombre + "\n" +
							"Firma del Responsable: ",
						style: 'itemsColumn'
					},
					_entExpPreviaTrabajo
				]
			});
			// ----------- FIN Entrenamiento / Experiencia previa en el trabajo  ----------------------------------------//
			//------------ conformidadEmpleado -------------------------------------------------------------------------//
			_contenido.push({
				text: "\n" + "Evaluación del Desempeño en la Operación",
				style: 'title'
			});
			_contenido.push({
				text: "\n" +
					"El Centro de Control de Operaciones considera adecuado el desempeño del operador",
				style: 'subTitle'
			});
			let _conformidadEmpleado = [];
			if (conformidadEmpleado.Habilitado_Firma !== "-") {
				_conformidadEmpleado.push({
					image: conformidadEmpleado.Habilitado_Firma,
					fit: [150, 150],
					style: 'signatureEntrenamiento'
				});
			}
			_contenido.push({
				columns: [{
						width: 'auto',
						text: "\n" +
							"Fecha: " + conformidadEmpleado.Habilitado_Fecha + "\n" +
							"Aclaración: " + conformidadEmpleado.Habilitado_Nombre + "\n" +
							"Firma del Responsable: ",
						style: 'itemsColumn'
					},
					_conformidadEmpleado
				]
			});
			//------------ FIN conformidad del Empleado ----------------------------------------------------------------//
			// -------   Evaluación del Desempeño en la Operación ---------- //
			_contenido.push({
				text: "\n" + "Evaluación del Desempeño en la Operación",
				style: 'title'
			});
			_contenido.push({
				text: "\n" +
					"El Centro de Control de Operaciones considera adecuado el desempeño del operador",
				style: 'subTitle'
			});
			let _evalDesempeño = [];
			if (evalDesempempeño.Capacitador_Firma !== "-") {
				_evalDesempeño.push({
					image: evalDesempempeño.Capacitador_Firma,
					fit: [150, 150],
					style: 'signatureEntrenamiento'
				});
			}
			_contenido.push({
				columns: [{
						width: 'auto',
						text: "\n" +
							"Fecha: " + evalDesempempeño.Capacitador_Fecha + "\n" +
							"Aclaración: " + evalDesempempeño.Capacitador_Nombre + "\n" +
							"Firma del Responsable: ",
						style: 'itemsColumn'
					},
					_evalDesempeño
				]
			});
			// ----------- FIN Evaluación del Desempeño en la Operación  ----------------------------------------//
			// -------   Medicina Laboral ---------------------------------------------------------------------- //
			_contenido.push({
				text: "\n" + "Medicina Laboral",
				style: 'title'
			});
			let _medicinaLaboral = [];
			if (gradoAptitud.Medicina_Firma !== "-") {
				_medicinaLaboral.push({
					image: gradoAptitud.Medicina_Firma,
					fit: [150, 150],
					style: 'signatureEntrenamiento'
				});
			}
			_contenido.push({
				columns: [{
						width: 'auto',
						text: "\n" +
							"Grado de Aptitud: " + gradoAptitud.GradoAp + "\n" +
							"Fecha de Vencimiento de Apto Médico: " + gradoAptitud.Medicina_Fecha_vencimiento + "\n" +
							"Fecha: " + gradoAptitud.Medicina_Fecha + "\n" +
							"Aclaración: " + gradoAptitud.Medicina_Nombre + "\n" +
							"Firma: ",
						style: 'itemsColumn'
					},
					_medicinaLaboral
				]
			});
			// ----------- FIN Medicina Laboral -----------------------------------------------------------------//
			// -------   Seguridad e Higiene ---------------------------------------------------------------------- //
			_contenido.push({
				text: "\n" + "Seguridad e Higiene",
				style: 'title'
			});
			let _segHigiene = [];
			if (segHigiene.SeguridadHigiene_Firma !== "-") {
				_segHigiene.push({
					image: segHigiene.SeguridadHigiene_Firma,
					fit: [150, 150],
					style: 'signatureEntrenamiento'
				});
			}
			_contenido.push({
				columns: [{
						width: 'auto',
						text: "\n" +
							"Aprobado: " + segHigiene.aprobadoCheck + "\n" +
							"Nro de veces reprobadas: " + segHigiene.CountEvaluation + "\n" +
							"Fecha de Examen: " + segHigiene.SeguridadHigiene_Fecha_examen + "\n" +
							"Se recomienda otorgar la Licencia Habilitante por el término de: " + segHigiene.SeguridadHigiene_Anio_Venc + " " + "/" + " " +
							segHigiene.SeguridadHigiene_Cantidad_Venc + " " + "/" + " " + segHigiene.SeguridadHigiene_Tiempo_Venc + "\n" +
							"Observaciones: " + segHigiene.SeguridadHigiene_Observacion + "\n" +
							"Aclaración: " + segHigiene.SeguridadHigiene_Nombre + "\n" +
							"Firma: ",
						style: 'itemsColumn'
					},
					_segHigiene
				]
			});
			// ----------- FIN Seguridad e Higiene -----------------------------------------------------------------//
			//------------ Evaluación y Calificación ---------------------------------------------------------------//
			_contenido.push({
				text: "\n" + "Evaluación y Calificación",
				style: 'title'
			});
			_contenido.push({
				text: "\n" +
					"Certificamos que el empleado cuyos datos personales figuran al inicio, aprobó el examen de conocimiento requerido para su Habilitación." +
					"\n" +
					"Equipo Examinador",
				style: 'subTitle'
			});
			let _evalClasificacion1 = [];
			if (evalClasificacion.Examinador1_Firma !== "-") {
				_evalClasificacion1.push({
					image: evalClasificacion.Examinador1_Firma,
					fit: [150, 150],
					style: 'signatureEntrenamiento'
				});
			}
			_contenido.push({
				columns: [{
						width: 'auto',
						text: "\n" +
							"Nombre y Apellido: " + evalClasificacion.Examinador1_Nombre + "\n" +
							"Fecha: " + evalClasificacion.Examinador1_Fecha + "\n" +
							"Firma",
						style: 'itemsColumn'
					},
					_evalClasificacion1
				]
			});
			//FIN  Evaluacion y clasificacion 1
			// Evaluacion y clasificacion 2
			let _evalClasificacion2 = [];
			if (evalClasificacion.Examinador2_Firma !== "-") {
				_evalClasificacion2.push({
					image: evalClasificacion.Examinador2_Firma,
					fit: [150, 150],
					style: 'signatureEntrenamiento'
				});
			}
			_contenido.push({
				columns: [{
						width: 'auto',
						text: "\n" +
							"Nombre y Apellido: " + evalClasificacion.Examinador2_Nombre + "\n" +
							"Fecha: " + evalClasificacion.Examinador2_Fecha + "\n" +
							"Firma",
						style: 'itemsColumn'
					},
					_evalClasificacion2
				]
			});
			// Evaluacion y clasificacion 3
			let _evalClasificacion3 = [];
			if (evalClasificacion.Examinador3_Firma !== "-") {
				_evalClasificacion3.push({
					image: evalClasificacion.Examinador3_Firma,
					fit: [150, 150],
					style: 'signatureEntrenamiento'
				});
			}
			_contenido.push({
				columns: [{
						width: 'auto',
						text: "\n" +
							"Nombre y Apellido: " + evalClasificacion.Examinador3_Nombre + "\n" +
							"Fecha: " + evalClasificacion.Examinador3_Fecha + "\n" +
							"Firma",
						style: 'itemsColumn'
					},
					_evalClasificacion3
				]
			});
			//------------ FIN Evaluación y Calificación -----------------------------------------------------------//
			//---------------- Gerencia Regional / Jefatura COT - COTDT ---------------------------------------------//
			_contenido.push({
				text: "\n" + "Gerencia Regional / Jefatura COT - COTDT",
				style: 'title'
			});
			// Evaluacion y clasificacion 3
			let _gerRegionalJefCOT = [];
			if (gerRegionalJefCOT.Gerente_Reg_Firma !== "-") {
				_gerRegionalJefCOT.push({
					image: gerRegionalJefCOT.Gerente_Reg_Firma,
					fit: [150, 150],
					style: 'signatureEntrenamiento'
				});
			}
			_contenido.push({
				columns: [{
						width: 'auto',
						text: "\n" +
							"Fecha: " + gerRegionalJefCOT.Gerente_Reg_Fecha + "\n" +
							"Se recomienda otorgar la Licencia Habilitante por el término de: " + gerRegionalJefCOT.Gerente_Reg_Anio_Venc + " " + "/" +
							" " +
							gerRegionalJefCOT.Gerente_Reg_Cantidad_Venc + " " + "/" + " " + gerRegionalJefCOT.Gerente_Reg_Tiempo_Venc + "\n" +
							"Observaciones: " + gerRegionalJefCOT.Gerente_Reg_Observacion + "\n" +
							"Aclaración: " + gerRegionalJefCOT.Gerente_Reg_Nombre + "\n" +
							"Firma",
						style: 'itemsColumn'
					},
					_gerRegionalJefCOT
				]
			});
			//---------------- fin Gerencia Regional / Jefatura COT - COTDT ---------------------------------------------//
			//----------------- Gerencia de Planificación y Operación de la Red -----------------------------------------//
			_contenido.push({
				text: "\n" + "Gerencia de Planificación y Operación de la Red",
				style: 'title'
			});
			// Evaluacion y clasificacion 3
			let _gerPlanOperRed = [];
			if (gerPlanOperRed.Ger_Operaciones_Firma !== "-") {
				_gerPlanOperRed.push({
					image: gerPlanOperRed.Ger_Operaciones_Firma,
					fit: [150, 150],
					style: 'signatureEntrenamiento'
				});
			}
			_contenido.push({
				columns: [{
						width: 'auto',
						text: "\n" +
							"Fecha: " + gerPlanOperRed.Ger_Operaciones_Fecha + "\n" +
							"Se recomienda otorgar la Licencia Habilitante por el término de: " + gerPlanOperRed.Ger_Operaciones_Anio_Venc + " " + "/" +
							" " +
							gerPlanOperRed.Ger_Operaciones_Cantidad_Venc + " " + "/" + " " + gerPlanOperRed.Ger_Operaciones_Tiempo_Venc + "\n" +
							"Observaciones: " + gerPlanOperRed.Ger_Operaciones_Observacion + "\n" +
							"Aclaración: " + gerPlanOperRed.Ger_Operaciones_Nombre + "\n" +
							"Firma",
						style: 'itemsColumn'
					},
					_gerPlanOperRed
				]
			});
			//----------------- FIN Gerencia de Planificación y Operación de la Red -----------------------------------------//
			//----------------- Gestión de la Calidad -----------------------------------------//
			_contenido.push({
				text: "\n" + "Gestión de la Calidad",
				style: 'title'
			});
			let _gestCalidad = [];
			if (gestCalidad.Gestion_Calidad_Firma !== "-") {
				_gestCalidad.push({
					image: gestCalidad.Gestion_Calidad_Firma,
					fit: [150, 150],
					style: 'signatureEntrenamiento'
				});
			}
			_contenido.push({
				columns: [{
						width: 'auto',
						text: "\n" +
							"Observaciones: " + gestCalidad.ObservacionGestionCalidad + "\n" +
							"Propuesta de Fecha de Vencimiento de la Habilitación: " + gestCalidad.Gestion_Calidad_Fecha_Habilitacion + "\n" +
							"Aclaración" + gestCalidad.Gestion_Calidad_Nombre + "\n" +
							"Firma",
						style: 'itemsColumn'
					},
					_gestCalidad
				]
			});
			//-----------------FIN Gestión de la Calidad -----------------------------------------//
			//-----------------Verificación de la Jefatura de Gestión de la Calidad -----------------------------------------//
			_contenido.push({
				text: "\n" + "Verificación de la Jefatura de Gestión de la Calidad",
				style: 'title'
			});
			_contenido.push({
				text: "\n" +
					"Se deja constancia de la verificación del cumplimiento de los requisitos de habilitación establecidos.",
				style: 'subTitle'
			});
			let _verJefaturaGyC = [];
			if (verJefaturaGyC.Rep_Direccion_Firma !== "-") {
				_verJefaturaGyC.push({
					image: verJefaturaGyC.Rep_Direccion_Firma,
					fit: [150, 150],
					style: 'signatureEntrenamiento'
				});
			}
			_contenido.push({
				columns: [{
						width: 'auto',
						text: "\n" +
							"Observaciones: " + verJefaturaGyC.Rep_Direccion_Observacion + "\n" +
							"Propuesta de Fecha de Vencimiento de la Habilitación: " + verJefaturaGyC.Rep_Direccion_Fecha + "\n" +
							"Aclaración" + verJefaturaGyC.Rep_Direccion_Nombre + "\n" +
							"Firma",
						style: 'itemsColumn'
					},
					_verJefaturaGyC
				]
			});
			//----------------- Conformidad de la Dirección Técnica -----------------------------------------//
			_contenido.push({
				text: "\n" + "Conformidad de la Dirección Técnica",
				style: 'title'
			});
			_contenido.push({
				text: "\n" +
					"Habiendo considerado todos los datos precedentes y demás información relacionada, se otorga la correspondiente licencia habilitante.",
				style: 'subTitle'
			});
			let _confDireccionTec = [];
			if (confDireTecnica.Direccion_Tecnica_Firma !== "-") {
				_confDireccionTec.push({
					image: confDireTecnica.Direccion_Tecnica_Firma,
					fit: [150, 150],
					style: 'signatureEntrenamiento'
				});
			}
			_contenido.push({
				columns: [{
						width: 'auto',
						text: "\n" +
							"Fecha de Vencimiento de la Habilitación: " + confDireTecnica.Direccion_Tecnica_Fecha_Vigencia + "\n" +
							"Fecha: " + confDireTecnica.Direccion_Tecnica_Fecha + "\n" +
							"Aclaración" + confDireTecnica.Direccion_Tecnica_Nombre + "\n" +
							"Firma",
						style: 'itemsColumn'
					},
					_confDireccionTec
				]
			});
			// ----------- FIN Seccion FINAL conformidad gerente Regional o gerente de Planificacion o Operacion de la red  --------------------//
			// Agregar el contenido al array final // Validando las imágenes por utlima vez
			// Agregar el contenido al array final // Validando las imágenes por utlima vez
			for (let i = 0; i < _contenido.length; i++) {
				if (_contenido[i].image !== "-") {
					_content.push(_contenido[i]);
				}
			};
			// Estructura del Formulario
			let docDefinition = {
				info: {
					title: 'LICENCIA HABILITATE (Personal de Operaciones)',
					author: 'Transener'
				},
				content: _content,
				styles: {
					header: {
						fontSize: 10,
						bold: true,
						alignment: 'center',
						top: 4
					},
					smallText: {
						fontSize: 9,
						top: 2
					},
					title: {
						fontSize: 10,
						bold: true,
						color: '#1060ba'
					},
					subHeaderr: {
						fontSize: 10,
						bold: false
					},
					anotherStyle: {
						color: '#515151',
						fontSize: 10,
						bold: true
					},
					defaultStyle: {
						top: 10
					},
					itemsColumn: {
						fontSize: 10,
						bold: true
					},
					signature: {
						color: '#000',
						fontSize: 10,
						bold: false,
						top: 0,
						alignment: 'center'
					},
					spacer: {
						top: 10,
						margin: 10
					},
					signatureEntrenamiento: {
						margin: [20, 0, 0, 0]
					}
				}
			};
			pdfMake.createPdf(docDefinition).open();
		}
	};
});